...
void CPlayer::MoveProcess(const std::vector<TmxObject> & collisionBlocks)
{
    ...
    {
        m_jumpCooldownSec = (m_jumpCooldownSec > 0) ? m_jumpCooldownSec - deltaSec : 0;
...
void CPlayer::AttackProcess(std::vector<CEnemy*> & enemies) const
{
    for (CEnemy * enemy : enemies)
    {
        if (attacking)
        {
            Collision doesTouchEnemy = DoesTouchEnemy(enemy);
            if ((m_lastDirection.x == 1 && doesTouchEnemy.right) || (m_lastDirection.x == -1 && doesTouchEnemy.left))
            {
                enemy->Die();
...
Collision CPlayer::DoesTouchEnemy(const CEnemy * enemy) const
{
    Collision result;

    float playerWidth = m_sprite.getGlobalBounds().width;
    float playerHeight = m_sprite.getGlobalBounds().height;

    float playerTopY = m_sprite.getPosition().y;
    float playerBottomY = m_sprite.getPosition().y + playerHeight;
    float playerLeftX = m_sprite.getPosition().x;
    float playerRightX = m_sprite.getPosition().x + playerWidth;
    ...
    {
        result.left = isLeftCollision;
        result.right = isRightCollision;
    }
    return result;
}
...
void CPlayer::UpdateDirection()
{
	...
	{
        if (m_jumpCooldownSec == 0 && canJump)
        {
            canJump = false;
            direction.y = -1;
        }
